...
int64_t fac[maxn], ifac[maxn], inv[maxn], h[maxn];

void solve(void) {
    int n = read();
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) ans = (ans + read() * (h[i] + h[n - i + 1] + mod - 1)) % mod;
    return write(ans * fac[n] % mod), putch('\n');
}
...
int main() {
    ...
    for (int i = 1; i < maxn; i++) inv[i] = ifac[i] * fac[i - 1];
    for (int i = 1; i < maxn; i++) h[i] = (h[i - 1] + inv[i]) % mod;

...
int main() {
    ...
    for (register int i = 1, t; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            ...
            dinic.addEdge(turn(i, j) + area, turn(i, j), INF);
            for (register int t = 0; t < 4; t++) {
                int ti = i + way[0][t], tj = j + way[1][t];
                if (1 <= ti && ti <= n && 1 <= tj && tj <= m) dinic.addEdge(turn(i, j) + area, turn(ti, tj), INF);
            }
        }
    for (register int i = 1, t; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            ...
            for (register int t = 0; t < 4; t++) {
                int ti = i + way[0][t], tj = j + way[1][t];
                if (1 <= ti && ti <= n && 1 <= tj && tj <= m) dinic.addEdge(turn(ti, tj), turn(i, j) + 2 * area, INF);
            }
        }

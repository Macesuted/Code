...
ZkwCostFlow<1005> web;

int main() {
    int n = read<int>(), m = read<int>(), S = n + 2, T = S + 1;
    web.INIT(T);
    for (register int i = 1; i <= n; i++) web.addEdge(i, i + 1, INF - read<int>(), 0);
    web.addEdge(S, 1, INF, 0), web.addEdge(n + 1, T, INF, 0);
    for (register int i = 1, s, t, c; i <= m; i++) {
        s = read<int>(), t = read<int>(), c = read<int>();
        web.addEdge(s, t + 1, INF, c);
    }
    web.minCostFlow(S, T);

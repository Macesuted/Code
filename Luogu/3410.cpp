...
int main() {
    ...
    for (register int i = 1, t; i <= n; i++) {
        dinic.addEdge(S, i, a[i] = read<int>());
        t = read<int>();
        while (t) dinic.addEdge(i, n + t, INF), t = read<int>();
    }
    for (register int i = 1; i <= m; i++) dinic.addEdge(n + i, T, b[i] = read<int>());
    dinic.maxFlow(S, T);
    long long answer = 0;
    for (register int i = 1; i <= n; i++)
        if (dinic.vis[i]) answer += a[i];
    for (register int i = 1; i <= m; i++)
        if (dinic.vis[n + i]) answer -= b[i];
    write(answer), putch('\n');
    return 0;
}
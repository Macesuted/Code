...
int main() {
    ...
    while (_--) {
        ...
        build(1, n);
        dfs2(1, 0);
        for (register int tx = 1; tx <= n; tx++)
            for (register int ty = tx + 1; ty <= n; ty++) {
                int x = tx, y = ty;
                long long ans = INF;
                if (dep[x] < dep[y]) swap(x, y);
                for (register int i = maxlgn - 1; ~i; i--)
                    if (dep[fa[x][i]] >= dep[y]) ans = min(ans, pre[x][i]), x = fa[x][i];
                if (x != y) {
                    for (register int i = maxlgn - 1; ~i; i--)
                        if (fa[x][i] != fa[y][i]) ans = min(ans, min(pre[x][i], pre[y][i])), x = fa[x][i], y = fa[y][i];
        ...
        int q = read<int>();
        while (q--) write(upper_bound(answer.begin(), answer.end(), read<long long>()) - answer.begin()), putch('\n');
        putch('\n');
        answer.clear(), graph.clear();
    }
    return 0;
}
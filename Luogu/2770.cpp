...
void dfs(int p, int id) {
    ...
    for (vector<ZkwCostFlow<long long, long long>::Edge>::iterator i = flow.graph[p + n].begin(); i != flow.graph[p + n].end(); i++)
...
int main() {
    ...
    int cnt = 0;
    for (vector<ZkwCostFlow<long long, long long>::Edge>::iterator i = flow.graph[1 + n].begin(); i != flow.graph[1 + n].end(); i++)

...
int main() {
    ...
    int d = read<int>(), tot = n * m, S = n * m * 2 + 1, T = S + 1;
    dinic.INIT(T);
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            char c = getch();
            while (c < '0' || c > '3') c = getch();
            if (c == '0') continue;
            dinic.addEdge(turn(i, j), turn(i, j) + tot, c - '0');
            has[i][j] = true;
        }
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++)
            if (has[i][j]) {
                if (i <= d || i + d > n || j <= d || j + d > m) dinic.addEdge(turn(i, j) + tot, T, INF);
                for (register int x = 1; x <= n; x++)
                    for (register int y = 1; y <= m; y++)
                        if (has[x][y] && (i - x) * (i - x) + (j - y) * (j - y) <= d * d)
                            dinic.addEdge(turn(i, j) + tot, turn(x, y), INF);
            }
    int cnt = 0;
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            char c = getch();
            while (c != '.' && c != 'L') c = getch();
            if (c == 'L') dinic.addEdge(S, turn(i, j), 1), cnt++;
        }
    write(cnt - dinic.maxFlow(S, T)), putch('\n');
    return 0;
}
...
class SegmentTree1 {
   ...
   public:
    ...
    inline int getMin(int l, int r) {
        if (l < 1 || l > n || r < 1 || r > n || l > r) cout << "C1" << endl, exit(0);
        return getMin(1, 1, n, l, r);
    }
    inline int getMax(int l, int r) {
        if (l < 1 || l > n || r < 1 || r > n || l > r) cout << "C2" << endl, exit(0);
        return getMax(1, 1, n, l, r);
    }
    inline void update(int p, int val) {
        if (p < 1 || p > n) cout << "C3" << endl, exit(0);
        return update(1, 1, n, p, val);
    }
};
class SegmentTree2 {
   private:
   ...
   public:
    ...
    inline int getGcd(int l, int r) {
        if (l < 1 || l > n || r < 1 || r > n || l > r) cout << "D1" << endl, exit(0);
        return getGcd(1, 1, n, l, r);
    }
    inline void update(int p, int val) {
        if (p < 1 || p > n) cout << "D2" << endl, exit(0);
        return update(1, 1, n, p, val);
    }
};
class SegmentTree3 {
   ...
   public:
    ...
    inline void build(int a[]) { return build(1, 1, n, a); }
    inline int getMax(int l, int r) {
        if (l < 1 || l > n || r < 1 || r > n || l > r) cout << "E1" << endl, exit(0);
        return getMax(1, 1, n, l, r);
    }
    inline void update(int p, int val) {
        if (p < 1 || p > n) cout << "E2" << endl, exit(0);
        return update(1, 1, n, p, val);
    }
};
...
int main() {
    ...
    while (m--) {
        if (read<int>() == 1) {
            int p = read<int>() ^ cnt, v = read<int>() ^ cnt;
            tree1.update(p, v);
            tree2.update(p, b[p] = abs(v - a[p - 1]));
            if (p < n) tree2.update(p + 1, b[p + 1] = abs(a[p + 1] - v));
            ...
            int l = read<int>() ^ cnt, r = read<int>() ^ cnt, k = read<int>() ^ cnt;
            if (l == r) {
                cnt++, putstr("Yes\n");
                continue;
            }
            ...
            if (up - down != 1LL * (r - l) * k) check = false;
            if (tree2.getGcd(l + 1, r) != k) check = false;
            if (k != 0 && tree3.getMax(l, r) >= l) check = false;
            if (check) cnt++;

...
inline bool check(int x, int y) { return 1 <= x && x <= n && 1 <= y && y <= m && c[x][y] == '.'; }
...
int main() {
    ...
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            c[i][j] = getch();
            while (c[i][j] != 'x' && c[i][j] != '.') c[i][j] = getch();
        }
    int tot = n * m, S = tot * 2 + 1, T = tot * 2 + 2;
    dinic.INIT(T);
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++) {
            dinic.addEdge(S, turn(i, j), 1), dinic.addEdge(turn(i, j) + tot, T, 1);
            if (c[i][j] == '.') {
                if (check(i + R, j + C)) dinic.addEdge(turn(i, j), turn(i + R, j + C) + tot, 1);
                if (check(i + R, j - C)) dinic.addEdge(turn(i, j), turn(i + R, j - C) + tot, 1);
                if (check(i + C, j + R)) dinic.addEdge(turn(i, j), turn(i + C, j + R) + tot, 1);
                if (check(i + C, j - R)) dinic.addEdge(turn(i, j), turn(i + C, j - R) + tot, 1);
            } else
                dinic.addEdge(turn(i, j), turn(i, j) + tot, 1);
        }
    write(n * m - dinic.maxFlow(S, T)), putch('\n');
    return 0;
}
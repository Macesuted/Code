...
const int way[2][8] = {{-1, 1, 2, 2, 1, -1, -2, -2}, {-2, -2, -1, 1, 2, 2, 1, -1}};

Dinic<maxn * maxn> dinic;
...
int main() {
    ...
    int S = n * n + 1, T = S + 1;
    dinic.INIT(T);
    for (register int i = 1; i <= m; i++) a[read<int>()][read<int>()] = true;
            ...
            if (!a[i][j]) {
                if ((i + j) & 1) {
                    ...
                    for (register int t = 0; t < 8; t++) {
                        ...
                        if (1 <= ti && ti <= n && 1 <= tj && tj <= n && !a[ti][tj])
                            dinic.addEdge(turn(i, j), turn(ti, tj), INF);
                    }
                } else
                    dinic.addEdge(turn(i, j), T, 1);
            }
    write(n * n - m - dinic.maxFlow(S, T)), putch('\n');
    return 0;
}
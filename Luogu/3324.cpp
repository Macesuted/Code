...
bool check(long long lim) {
    ...
    long long ans = 0;
    dinic.INIT(T);
    for (register int i = 1; i <= m; i++) dinic.addEdge(S, i, b[i] * lim);
    for (register int i = 1; i <= n; i++) dinic.addEdge(i + m, T, a[i] * 10000LL), ans += a[i] * 10000LL;
    for (register int i = 1; i <= m; i++)
        for (register int j = 1; j <= n; j++)
            if (alow[i][j]) dinic.addEdge(i, m + j, INF);
    bool ret = (dinic.maxFlow(S, T) == ans);
    dinic.clear();
    return ret;
}
...
int main() {
    ...
    for (register int i = 1; i <= m; i++) b[i] = read<int>();
    for (register int i = 1; i <= m; i++)
        for (register int j = 1; j <= n; j++) alow[i][j] = read<int>();

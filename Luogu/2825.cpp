...
int main() {
    ...
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= m; j++)
            while (a[i][j] != '#' && a[i][j] != '*' && a[i][j] != 'x') a[i][j] = getch();
    int S = 3 * n * m + 1, T = S + 1;
    dinic.INIT(T);
    for (register int i = 1; i <= n; i++)
        for (register int j = 1, last = 0; j <= m; j++) {
            if (a[i][j] == '#')
                ...
                dinic.addEdge(S, (last = turn(i, j)) + n * m, 1);
            if (a[i][j] == '*') dinic.addEdge(last + n * m, turn(i, j), 1);
        }
    for (register int j = 1; j <= m; j++)
        for (register int i = 1, last = 0; i <= n; i++) {
            if (a[i][j] == '#')
                ...
                dinic.addEdge((last = turn(i, j)) + 2 * n * m, T, 1);
            if (a[i][j] == '*') dinic.addEdge(turn(i, j), last + 2 * n * m, 1);
        }

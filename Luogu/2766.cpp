...
int main() {
    ...
    write(ans), putch('\n');
    for (register int i = 1; i <= n; i++)
        if (f[i] == 1) dinic.addEdge(S, i, 1);
    for (register int i = 1; i <= n; i++)
        if (f[i] == ans) dinic.addEdge(i + n, T, 1);
    for (register int i = 1; i <= n; i++) dinic.addEdge(i, i + n, 1);
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j < i; j++)
            if (a[j] <= a[i] && f[j] + 1 == f[i]) dinic.addEdge(j + n, i, 1);
    write(dinic.maxFlow(S, T)), putch('\n');
    if (f[1] == 1) dinic.addEdge(S, 1, INF);
    if (n != 1 && f[n] == ans) dinic.addEdge(n + n, T, INF);
    dinic.addEdge(1, 1 + n, INF), dinic.addEdge(n, n + n, INF);
    write(dinic.maxFlow(S, T)), putch('\n');
    return 0;
}
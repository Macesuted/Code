...
const int way[2][8] = {{-2, -2, -1, 1, 2, 2, 1, -1}, {-1, 1, 2, 2, 1, -1, -2, -2}};
...
inline int turn(int x, int y) { return (x - 1) * n + y; }
inline bool check(int x, int y) { return 1 <= x && x <= n && 1 <= y && y <= n && c[x][y] == '0'; }
...
int main() {
    ...
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= n; j++) {
            c[i][j] = getch();
            while (c[i][j] != '0' && c[i][j] != '1') c[i][j] = getch();
        }
    int S = n * n + 1, T = n * n + 2;
    dinic.INIT(T);
    for (register int i = 1; i <= n; i++)
        for (register int j = 1; j <= n; j++)
            if (c[i][j] == '0') {
                if ((i + j) & 1) {
                    dinic.addEdge(S, turn(i, j), 1);
                    for (register int t = 0; t < 8; t++) {
                        int ti = i + way[0][t], tj = j + way[1][t];
                        if (check(ti, tj)) dinic.addEdge(turn(i, j), turn(ti, tj), INF);
                    }
                } else
                    dinic.addEdge(turn(i, j), T, 1);
            } else
                dinic.addEdge(S, turn(i, j), 1), dinic.addEdge(turn(i, j), T, 1);
    write(n * n - dinic.maxFlow(S, T)), putch('\n');
    return 0;
}
...
int main() {
    int n = read<int>(), m = read<int>(), k = read<int>(), S = 2 * n + 1;
    dinic.INIT(S);
    for (register int i = 1, from, to; i <= m; i++) {
        from = read<int>(), to = read<int>();
        dinic.addEdge(from + n, to, INF), dinic.addEdge(to + n, from, INF);
    }
    for (register int i = 1; i <= k; i++) vis[read<int>()] = true;
    for (register int i = 1; i <= n; i++)
        if (vis[i])
            dinic.addEdge(S, i, INF), dinic.addEdge(i, n + i, INF);
        else
            dinic.addEdge(i, n + i, 1);
    write(dinic.maxFlow(S, 1)), putch('\n');
    return 0;
}
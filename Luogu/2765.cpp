...
int main() {
    int n = read<int>();
    int num = 1, S = 1, T = 2, sum = 0;
    dinic.INIT(30005);
    while (true) {
        dinic.addEdge(S, 2 * num + 1, 1), dinic.addEdge(2 * num + 2, T, 1);
        for (register int i = 2 * sqrt(num); i; i--) {
            int j = i * i - num;
            if (0 < j && j < num) dinic.addEdge(2 * j + 1, 2 * num + 2, 1);
        }
    ...
    for (register int i = 1; i <= num; i++)
        if (!vis[i]) {
            for (register int j = i; j; j = aft[j]) vis[j] = true, write(j), putch(' ');
            putch('\n');
        }
    return 0;
}
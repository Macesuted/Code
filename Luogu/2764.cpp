...
void print(int p) {
    write(p), putch(' ');
    for (vector<Dinic<1005>::Edge>::iterator i = dinic.graph[p].begin(); i != dinic.graph[p].end(); i++)
        if (i->real && i->flow == i->cap) print(i->to - n);
    return;
}
...
int main() {
    ...
    for (register int i = 1; i <= n; i++)
        for (vector<Dinic<1005>::Edge>::iterator j = dinic.graph[i].begin(); j != dinic.graph[i].end(); j++)
            if (j->real && j->flow == j->cap) fa[getfa(j->to - n)] = getfa(fa[i]);
    for (register int i = 1; i <= n; i++)
        if (fa[i] == i) print(i), putch('\n');
    write(n - ret), putch('\n');
    return 0;
}
...
void solve(void) {
    ...
    for (int i = 1; i <= m; i++) t[read()] ^= true;
    m = 0;
    for (int i = 1; i < maxn; i++)
        if (t[i]) b[++m] = i;
    for (int i = 1; i <= q; i++) ques[i] = {read(), i};
    sort(b + 1, b + m + 1, greater<int>()), sort(ques + 1, ques + q + 1, greater<pii>());
    for (int i = 1, j = 1; i <= m + 1; i++) {
        while (j <= q && ques[j].first >= b[i]) ans[ques[j].second] = (S & (T << ques[j].first)).count() & 1, j++;
        if (i == m + 1) break;
        for (int x = 0; x < maxn; x += b[i]) T[x] = !T[x];
    }
    for (int i = 1; i <= q; i++) write((int)ans[i]), putch('\n');
    return;
}

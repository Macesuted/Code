...
set<int> S;
void dfs(int p) {
    S.insert(p), vis[p] = true;
    for (int i = 0; i < 4; i++)
        if (!S.count(i) && sum[min(p, i)][max(p, i)]) dfs(i);
    return;
}

void solve(void) {
    int n = read(), minVal = INT_MAX;
    for (int i = 1; i <= n; i++) {
        int d = read();
        char x = getstr()[0], y = getstr()[0];
        if (x > y) swap(x, y);
        int tx = x - 'W', ty = y - 'W';
        minVal = min(minVal, d), sum[tx][ty] += d, deg[tx]++, deg[ty]++;
    }
    int ans = 0;
    for (int i = 0; i < 4; i++) {
        if (vis[i]) continue;
        ...
        for (auto i = S.begin(); i != S.end(); i++)
            for (auto j = i; j != S.end(); j++) tot += sum[*i][*j];
        for (auto i : S) cnt += deg[i] & 1;
        if (cnt == 4) tot -= minVal;
        ans = max(ans, tot);
    }
    write(ans), putch('\n');

...
int64_t fac[maxn * 2], ifac[maxn * 2], f[maxn][maxn], g[maxn][maxn * 2];
...
void solve(void) {
    ...
    for (int i = maxn * 2 - 2; i; i--) ifac[i] = ifac[i + 1] * (i + 1) % mod;
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j <= i; j++)
            for (int y = max(1, j); y <= n - i; y++)
                if (!((j + y) & 1))
                    f[i + y][(j + y) / 2] =
                        (f[i + y][(j + y) / 2] + f[i][j] * fac[n - i] % mod * ifac[n - i - y] % mod * ifac[y]) % mod;
        for (int j = 0; j <= i; j++)
            for (int x = 1; x <= n - i; x++)
                g[x + i][x - j + n] = (g[x + i][x - j + n] + f[i][j] * fac[n - i] % mod * ifac[x]) % mod;
    }
    write(f[n][0]), putch('\n');
    return;
}

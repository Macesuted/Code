...
void dfs(int l, int r, int pre = 0) {
    if (l > r) return;
    if (l == r) return fa[l] = pre, void();
    int p = f[l][r].second;
    fa[p] = pre;
    dfs(l, p - 1, p), dfs(p + 1, r, p);
    return;
}
    ...
    for (int len = 2; len <= n; len++)
        for (int l = 1, r = len; r <= n; l++, r++) {
            f[l][r] = min(pli{f[l][r - 1].first + sum(l, r - 1, 1, l - 1) + sum(l, r - 1, r, n), r},
                          pli{f[l + 1][r].first + sum(l + 1, r, 1, l) + sum(l + 1, r, r + 1, n), l});
            for (int p = l + 1; p < r; p++)
                f[l][r] = min(f[l][r], pli{f[l][p - 1].first + sum(1, l - 1, l, p - 1) + sum(l, p - 1, p, n) +
                                               sum(1, p, p + 1, r) + sum(p + 1, r, r + 1, n) + f[p + 1][r].first,
                                           p});
        }
    dfs(1, n);
    for (int i = 1; i <= n; i++) write(fa[i]), putch(' ');
    putch('\n');
    return;
}

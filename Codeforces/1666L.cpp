...
int fa[maxn], vis[maxn], S;

vector<int> getPath(int p) {
    vector<int> path;
    while (p) path.push_back(p), p = fa[p];
    return reverse(path.begin(), path.end()), path;
}

void dfs(int p, int col, int fa_) {
    if (p == S || vis[p] == col) return;
    if (!vis[p]) vis[p] = col, fa[p] = fa_;
    if (vis[p] != col) {
        putstr("Possible\n");
        vector<int> a = getPath(p), b = getPath(fa_);
        write(a.size()), putch('\n');
        for (auto i : a) write(i), putch(' ');
        putch('\n'), write(b.size() + 1), putch('\n');
        for (auto i : b) write(i), putch(' ');
        write(p), putch('\n');
        exit(0);
    }
    for (auto i : graph[p]) dfs(i, col, p);
    return;
}

...
double p[maxn][maxn], f[maxn], prod[maxn];
bool vis[maxn];

void solve(void) {
    int n = read();
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++) p[i][j] = read() * 0.01;
    for (int i = 1; i < n; i++) f[i] = 0, prod[i] = 1;
    f[n] = -1, prod[n] = 0;

    auto calc = [&](int p) { return prod[p] == 1 ? 1e18 : (f[p] + 1) / (1 - prod[p]); };

    while (!vis[1]) {
        int x = 1;
        for (int i = 2; i <= n; i++)
            if (!vis[i] && calc(i) < calc(x)) x = i;
        vis[x] = true, f[x] = calc(x);
        for (int i = 1; i <= n; i++)
            if (!vis[i]) f[i] += prod[i] * p[i][x] * f[x], prod[i] *= 1 - p[i][x];
    }
    writeDouble(f[1], 10), putch('\n');
    return;
}

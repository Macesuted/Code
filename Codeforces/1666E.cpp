...
int a[maxn], pl[maxn], pr[maxn], p[maxn];
...
void solve(void) {
    ...
    auto check = [&](int minLen) {
        auto chk = [&](int maxLen) {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                if (pl[i] > a[i]) return false;
                pl[i] = max(a[i], pl[i] + minLen), pr[i] = min(pr[i], a[i]) + maxLen;
                if (pr[i] < a[i] || pl[i] > pr[i]) return false;
            }
            return pl[n] <= L && L <= pr[n];
        };
        if (!chk(L)) return 0;
        int l = minLen - 1, r = L + 1;
        while (l + 1 < r) {
            int mid = (l + r) >> 1;
            chk(mid) ? r = mid : l = mid;
        }
    ...
    r = check(l);
    p[n] = L;
    for (int i = n - 1; i; i--) p[i] = max(pl[i], p[i + 1] - r);
    for (int i = 0; i < n; i++) write(p[i]), putch(' '), write(p[i + 1]), putch('\n');
    return;
}

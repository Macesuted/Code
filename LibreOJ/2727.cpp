...
int n, m, gn, d[maxn], p[maxn], v[maxn], f[maxn];

vector<vector<int>> graph;

bool check(int lim) {
    int cnt = 0;
    for (int i = m + 1; i <= n; i++) cnt += (d[i] >= lim);
    for (int i = 1; i <= n; i++) f[i] = 1;
    for (int i = 1; i <= m; i++) f[p[i]] = (d[i] >= lim ? 0 : n);
    for (int i = n + 1; i <= gn; i++)
        f[i] = min({f[graph[i][0]] + f[graph[i][1]], f[graph[i][0]] + f[graph[i][2]], f[graph[i][1]] + f[graph[i][2]]});
    return f[gn] <= cnt;
}

void solve(void) {
    n = read(), m = read(), gn = n + n / 2;
    for (int i = 1; i <= n; i++) {
        d[i] = v[i] = read();
        if (i <= m) p[i] = read();
    }
    ...
    while (que.size() > 1) {
        gcnt++;
        for (int i = 0; i < 3; i++) graph[gcnt].push_back(que.front()), que.pop();
        que.push(gcnt);
    }
    if (check(v[n])) return write(v[n]), putch('\n');
    int l = 1, r = n;
    while (l + 1 < r) {
        int mid = (l + r) >> 1;
        check(v[mid]) ? l = mid : r = mid;
    }
    return write(v[l]), putch('\n');
}

...
void solve(void) {
    int n = read();
    for (int i = 1; i <= n; i++) sum[i] = read(), g[i].clear();
    for (int i = 1; i < n; i++) a[i] = sum[i + 1] - sum[i];
    f[0] = 1;
    for (int i = 1; i < n; i++) {
        f[i] = f[g[i][a[i]] = i - 1] + 1;
        for (int x = a[i]; g[g[i][x]].find(x) != g[g[i][x]].end(); x = x + x)
            f[i] = min(f[i], f[g[i][x + x] = g[g[i][x]][x]] + 1);
    }
    return write(f[n - 1]), putch('\n');
}

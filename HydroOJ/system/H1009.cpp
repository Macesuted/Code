...
int check(int p) {
    ...
    for (int i = 0; i < p; i++)
        if (!S.empty() && S.back() == s[i])
            cnt++;
        else if (S.size() > 1 && S[(int)S.size() - 2] == s[i])
            S.pop_back(), cnt += 2;
        else
            S.push_back(s[i]), cnt += 2;
    for (int i = p; i < (int)s.size(); i++, cnt++) {
        while (!S.empty() && S.back() != s[i]) S.pop_back(), cnt++;
        if (S.empty()) return -1;
    }
    return cnt + S.size();
}
